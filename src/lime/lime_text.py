...
from lime import DomainMapper, Explanation, LimeBase
...
class IndexedString(object):
    ...
    # ThangPM: FOR LIME-BERT
    def inverse_masking(self, words_to_replace, masked_token):
        """Returns a string after replacing the appropriate words with a masked token (e.g., [MASK] for BERT or <mask> for RoBERTa).

        If self.bow is false, replaces word with UNKWORDZ instead of replacing it.

        Args:
            words_to_replace: list of ids (ints) to replace

        Returns:
            original raw string with appropriate words replaced.
        """
        mask = np.ones(self.as_np.shape[0], dtype='bool')
        mask[self.__get_idxs(words_to_replace)] = False
        if not self.bow:
            return ''.join([self.as_list[i] if mask[i] else self.mask_string for i in range(mask.shape[0])])
        return ''.join([self.as_list[idx] if idx in mask.nonzero()[0] else masked_token for idx in range(len(mask))])
...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
        ...
        indexed_string = (IndexedCharacters(text_instance, bow=self.bow, mask_string=self.mask_string) if self.char_level else
        ...
        data, yss, distances = self.__data_labels_distances(indexed_string, classifier_fn, num_samples,
                                                            ...
                                                            use_mlm=use_mlm, model_wrapper=model_wrapper)   # ThangPM
        ...
        for label in labels:
            (ret_exp.intercept[label], ret_exp.local_exp[label],
             ret_exp.score[label], ret_exp.local_pred[label]) = self.base.explain_instance_with_data(data, yss, distances, label, num_features,
    ...
    def __data_labels_distances(self,
        ...
        for i, size in enumerate(sample, start=1):
            ...
            # Original LIME
            if not use_mlm: inverse_data.append(indexed_string.inverse_removing(inactive))
            # LIME-BERT
            # Replace inactive tokens by masked tokens (e.g., [MASK]/<mask>)
            else: masked_data.append(indexed_string.inverse_masking(inactive, model_wrapper.mask_token))

        # ThangPM: Add filled examples to the list of inverse_data
        if use_mlm:
            # filled_data = model_wrapper.predict_multi_blanks_as_masked_lm(masked_data) # Independently
            filled_data = model_wrapper.predict_multi_blanks_as_masked_lm_batch_conditionally(masked_data) # Dependently
            inverse_data.extend(filled_data)
