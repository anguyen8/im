        ...
        for key, value in kwargs.items():
            if key in self.SPECIAL_TOKENS_ATTRIBUTES:
                if key == "additional_special_tokens":
                    ...
                    setattr(self, key, value)
                # ThangPM added here for running finetuned RoBERTa-large on combination of NLI datasets
                # For ANLI authors' finetuned RoBERTa-large model ONLY.
                elif isinstance(value, dict):
                    setattr(self, key, value["content"])
                else:
                    raise TypeError(
...
class PreTrainedTokenizer(SpecialTokensMixin):
            ...
            if os.path.isfile(pretrained_model_name_or_path) or is_remote_url(pretrained_model_name_or_path):
                ...
                for file_id, file_name in {**cls.vocab_files_names, **additional_files_names}.items():
                    if os.path.isdir(pretrained_model_name_or_path):
                        ...
                        # full_file_name = hf_bucket_url(pretrained_model_name_or_path, filename=file_name, use_cdn=False)
                        full_file_name = hf_bucket_url(pretrained_model_name_or_path, filename=file_name, revision=None, mirror=None)
    ...
    def encode_plus(
        ...
        **kwargs
        ...
        # ThangPM: Note 30-03-21
        # If len(tokens) exceeds max_length, truncate the beginning tokens to make it equal to `max_length`
        # if max_length != None and "mask_id" in kwargs["kwargs"]:
        #     if len(tokens) >= max_length and tokens.index(kwargs["kwargs"]["mask_id"]) >= max_length:
        #         tokens = tokens[len(tokens) - max_length:len(tokens)]
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        def get_input_ids(text):
            # ThangPM: Encode text to avoid encoding error when using only string
