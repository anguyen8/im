...
class Trainer:
    ...
    def predict_masked_lm(self, test_dataset: Dataset) -> PredictionOutput:
        """
        Run prediction and return predictions for [MASK] sentences
        ...
        return self._prediction_loop_masked_lm(test_dataloader, description="Prediction")

    def _prediction_loop_masked_lm(
        self, dataloader: DataLoader, description: str, prediction_loss_only: Optional[bool] = None,
    ) -> PredictionOutput:
        """
        Prediction/evaluation loop, shared by `evaluate()` and `predict()`.
        ...
        batch_size = dataloader.batch_size
        logger.info("***** Running Masked Language Model %s *****", description)
        logger.info("  Num examples = %d", self.num_examples(dataloader))
        logger.info("  Batch size = %d", batch_size)
        preds: torch.Tensor = None
        model.eval()
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            if preds is None:
                ...
                preds = torch.cat((preds, outputs[0].detach().cpu()), dim=0)
        ...
        return PredictionOutput(predictions=preds, label_ids=None, metrics=None, ml_objects=None)
